...
Public Class ClassRawFile
    ...
    Structure XDataType
        ...
        Dim EventName() As String ' Event Name OR MappingEvent - Range (1, NumOfEvent)
        Dim EventProb() As Single ' Event 확률  - Range (1, NumOfEvent)

        ' database & fragility in mdb
        Dim EventProbR() As Single ' Event 확률 in mdb  OR Am - Range (1, NumOfEvent) 
        Dim EventLamda() As Single ' Lamda  - Range (1, NumOfEvent)
        Dim EventTau() As Single ' Tau  - Range (1, NumOfEvent)
        Dim EventTauUnit() As String ' Tau unit d,h,m  - Range (1, NumOfEvent)
        Dim EventFactor() As Single ' Factor (CCF)  - Range (1, NumOfEvent) 
        Dim EventDistrN() As String   ' Event Distribution Name or CorrelationEvent
        Dim EventDistrP1() As Single   ' Event Distribution Parameter 1 OR Br
        Dim EventDistrP2() As Single   ' Event Distribution Parameter 2 OR Bu

        Dim index() As Integer
        Dim NumOfCCF, NumOfSeis, NumOfR As Integer
    ...
    End Structure
    Structure XHazardType ' 각 Hazard 의 data를 기록
        Dim Haz() As String ' Hazard id
        Dim Weight() As Single ' hazard 별 가중치
        Dim Acc() As Single ' hazard별 Acc
        Dim Freq(,) As Single ' hazard 별 freq
        Dim NumOfAcc As Integer 'Acc 갯수
        Dim NumOfHaz As Integer 'Hazard 갯수
    End Structure

    Public XData As New XDataType       ' Event 확률 저장용
    Public XCutSet As New XCutSetType   ' Cut Set 저장용
    Public XHazard As New XHazardType   ' Hazard 저장용
    ...
    Public Sub ReadRawFile(ByRef RawFileName As String)
        ...
        ReDim XData.EventLamda(XData.NumOfEvent)
        ReDim XData.EventTau(XData.NumOfEvent)
        ReDim XData.EventTauUnit(XData.NumOfEvent)
        ReDim XData.EventFactor(XData.NumOfEvent)
        ...
        For i = 1 To XData.NumOfEvent
            Message.Add(i.ToString & "    " & XData.EventName(i) & "    " & XData.EventProb(i))
        Next
        Message.Show()
        ...
        S1 = InputBox("CCF Pattern")
        S2 = InputBox("SEISMIC Pattern")

        ReDim XData.index(XData.NumOfEvent)

        For i = 1 To XData.NumOfEvent
            If (XData.EventName(i).Contains(S1)) Then
                XData.index(i) = 2 'CCF 기본사건 = 2
            ElseIf (XData.EventName(i).Contains(S2)) Then
                XData.index(i) = 3 '지진 기본사건 = 3
            Else
                XData.index(i) = 1 'Random 기본사건 = 1
            End If
            Console.WriteLine(XData.EventName(i).ToString & "  " & XData.index(i).ToString)
        Next

        Call Sub() SortingCutset()
    ...
    Public Sub SortingCutset()
        ...
        'For i = 1 To XData.NumOfEvent
        '    If (XData.EventName(i).Contains(S1)) Then
        '        XData.NumOfCCF = XData.NumOfCCF + 1
        '        XData.index(i) = XData.NumOfCCF
        '    ElseIf (XData.EventName(i).Contains(S2)) Then
        ...
        'If XData.NumOfCCF + XData.NumOfSeis + XData.NumOfR = XData.NumOfEvent Then
        '    For i = 1 To XData.NumOfEvent
        '        If (XData.EventName(i).Contains(S1)) Then
        '            XData.index(i) = temp(i)
        '        ElseIf (XData.EventName(i).Contains(S2)) Then
        '            XData.index(i) = XData.NumOfCCF + temp(i)
        '        Else
        '            XData.index(i) = XData.NumOfCCF + XData.NumOfSeis + temp(i)
        '        End If
        '    Next
    ...
    Public Sub ReadDataFile(ByRef DataFileName As String)
        ...
        Dim EvtLamda() As Single
        Dim EvtTau() As Single
        Dim EvtTauUnit() As String
        Dim EvtFactor() As Single
        ...
        ReDim EvtLamda(arrCount)
        ReDim EvtTau(arrCount)
        ReDim EvtTauUnit(arrCount)
        ReDim EvtFactor(arrCount)
        ...
        For index = 0 To arrCount - 1
            ...
            EvtFactor(index + 1) = myDS.Tables(0).Rows(index).Item(8)
        ...
        For index = 1 To arrCount
            ...
            If (iE = 9900) Then
                s = "No in MCS = " & Evtname(index) & vbTab & vbTab & pE
                Message.Add(s)
                GoTo n1
            End If

            XData.EventProbR(iE) = EvtProb(index)
            XData.EventFactor(iE) = EvtFactor(index)
            ...
            ' mdb 의 값이 정상적이 않음. lognormal 분포 기본사건 지정 - random sampling 과정의 에러를 방지하기 위한 조건식임.
...
Err_Renamed:
    ...
    End Sub
    ' mdb File 읽기 - Fragility
    Public Sub ReadDataFileF(ByRef DataFileName As String)
        ...
        Dim SeisIndex() = {"S015_", "S03_", "S05_", "S07_", "S09_", "S10_"} '지진구간별 index, S015, S03...
        ...
        'dbTable = InputBox("Table") '테이블 이름 지정
        dbTable = "ComponentFragilityInput"           '테이블 이름 지정
        columnNames = "SeismicEvent, CorrelationEvent, Component, MappingEvent, Am, Br, Bu, CCFGroup"
        dbTableCommand = "Select " & columnNames & " from " & dbTable
        MyRs.Open(dbTableCommand, MyCn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockOptimistic)
        ...
        ReDim Evtname(arrCount) 'MappingEvent
        ReDim EvtProb(arrCount) 'Am
        ReDim EvtDistrN(arrCount) 'CorrelationEvent
        ReDim EvtDistrP1(arrCount) 'Br
        ReDim EvtDistrP2(arrCount) 'Bu
        ...
        For index = 0 To arrCount - 1
            If ((DBNull.Value.Equals(myDS.Tables(0).Rows(index).Item(3)))) Then
                Evtname(index + 1) = "XXX"
            Else
                Evtname(index + 1) = myDS.Tables(0).Rows(index).Item(3)
            End If
            Evtname(index + 1) = Trim(Evtname(index + 1))

            If ((DBNull.Value.Equals(myDS.Tables(0).Rows(index).Item(1)))) Then
                EvtDistrN(index + 1) = "XXX"
            Else
                EvtDistrN(index + 1) = myDS.Tables(0).Rows(index).Item(1)
            End If
            EvtDistrN(index + 1) = Trim(EvtDistrN(index + 1))

            EvtProb(index + 1) = myDS.Tables(0).Rows(index).Item(4)
            EvtDistrP1(index + 1) = myDS.Tables(0).Rows(index).Item(5)
            EvtDistrP2(index + 1) = myDS.Tables(0).Rows(index).Item(6)
        Next
        MyRs = Nothing
        MyCn.Close()
        MyCn = Nothing
        ...
        '        For sin = 0 To UBound(SeisIndex)

        '            For index = 1 To arrCount
        '                pE = EvtProb(index)    ' Proba
        '                iE = GetIndexOfEvent(SeisIndex(sin) & Evtname(index)) ' Event Index
        '                If (iE = 9900) Then
        '                    iE = GetIndexOfEvent(SeisIndex(sin) & EvtDistrN(index)) ' Event Index
        '                    If (iE = 9900) Then
        '                        s = "No in MCS = " & SeisIndex(sin) & Evtname(index) & vbTab & vbTab & pE
        '                        Message.Add(s)
        '                        GoTo n1
        '                    End If
        '                End If

        '                XData.EventProbR(iE) = EvtProb(index) 'Am
        '                XData.EventDistrP1(iE) = EvtDistrP1(index) 'Br
        '                XData.EventDistrP2(iE) = EvtDistrP2(index) 'Bu

        '                s = SeisIndex(sin) & XData.EventName(iE) & " - " & XData.EventProb(iE).ToString & " - " & XData.EventDistrP1(iE).ToString
        '                Message.Add(s)
        'n1:
        '            Next
        '        Next
        For index = 1 To arrCount '확인용 HKL
            pE = EvtProb(index)    ' Proba
            iE = GetIndexOfEvent(Evtname(index)) ' Event Index
            If (iE = 9900) Then
                s = "No in MCS = " & Evtname(index) & vbTab & vbTab & pE
            ...
            s = XData.EventName(iE) & " - " & XData.EventProbR(iE).ToString & " - " & XData.EventDistrP1(iE).ToString & " - " & XData.EventDistrP2(iE).ToString & " - " & XData.index(iE).ToString
            Message.Add(s)
n1:
        Next
        Message.Show()
...
Err_Renamed:
        MsgBox("Fails to Read Fragility File")

    End Sub
    ' mdb File 읽기 - Hazard
    Public Sub ReadDataFileH(ByRef DataFileName As String)
        ...
        'dbTable = InputBox("Table") '테이블 이름 지정
        dbTable = "SeismicGroundinput"           '테이블 이름 지정
        columnNames = "SeismicGroup, RepresentAcc, MinAcc, MaxAcc, Frequency" 'SeismicGroup, RepresentAcc, MinAcc, MaxAcc, Frequency
        dbTableCommand = "Select " & columnNames & " from " & dbTable
        'dbTableCommand = "Select * from " & dbTable
        MyRs.Open(dbTableCommand, MyCn, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockOptimistic)
        ...
        ReDim Evtname(arrCount) 'SeismicGroup
        ReDim EvtProb(arrCount) 'Frequency
        ReDim EvtDistrP1(arrCount) 'MinAcc
        ReDim EvtDistrP2(arrCount) 'MaxAcc
        ...
        For index = 0 To arrCount - 1
            ...
            Evtname(index + 1) = Trim(Evtname(index + 1))
            EvtProb(index + 1) = myDS.Tables(0).Rows(index).Item(4)
            EvtDistrP1(index + 1) = myDS.Tables(0).Rows(index).Item(2)
            EvtDistrP2(index + 1) = myDS.Tables(0).Rows(index).Item(3)
        Next
        MyRs = Nothing
        MyCn.Close()
        MyCn = Nothing
...
Err_Renamed:
        MsgBox("Fails to Read Hazard File")
    ...
    Public Sub CalculateMonte(NoSamples As Integer, FileName As String)
        ...
        Dim Qrand, arand, xrand, invphiQ, phix As Single
        arand = UniFor(0.1, 2.0) 'random acceleration in Fragility - 0.1g to 2.0g 

        Dim ia As Integer
        For ia = 1 To 20 '임시로 : acc에 대하여 취약도합성
            arand = ia * 0.1

            For i = 1 To NoSamples
                For j = 1 To XData.NumOfEvent
                    If (XData.index(j) = 1) Then
                        If XData.EventDistrN(j) = "L" Then
                        ...
                        Message.Add(sx)
                    End If
                    If (XData.index(j) = 2) Then
                        XData.EventProb(j) = XData.EventDistrP1(j) * XData.EventDistrP2(j)
                    End If
                    If (XData.index(j) = 3) Then
                        Qrand = UniFoR(0, 1)
                        XData.EventProb(j) = FragilityEq(XData.EventProbR(j), XData.EventDistrP1(j), XData.EventDistrP2(j), Qrand, arand)
                    End If
                Next

                uncerR(i - 1) = CalculateCutSetsProba()

                sx = arand.ToString & vbTab & " cdf | " & uncerR(i - 1).ToString
            ...
            For i = 0 To NoSamples - 1
                sum = Math.Pow((uncerR(i) - Mean), 2) + sum
            Next
            ...
            oSheet.Cells(1, ia + 4).Value = Mean
            oSheet.Cells(2, ia + 4).Value = Sd
            oSheet.Cells(4, ia + 4).Value = Umax
            oSheet.Cells(5, ia + 4).Value = Umin
            oSheet.Cells(6, ia + 4).Value = P5
            oSheet.Cells(7, ia + 4).Value = P50
            oSheet.Cells(8, ia + 4).Value = P95
        Next
...
Err_Renamed:
    ...
    End Sub
    Public Function FragilityEq(ByVal RAm As Single, ByVal RBr As Single, ByVal RBu As Single, ByVal Qrand As Single, ByVal arand As Single) As Single
        Dim xrand, invphiQ, phix As Single
        Dim sx As String

        If (Qrand >= 0.5) Then
            invphiQ = 10.0 / Math.Log10(41) * Math.Log10(1 - Math.Log10((-Math.Log10(Qrand) / Math.Log10(2))) / Math.Log10(22)) 'Applied Mathematical Sciences, Vol. 8, 2014, no. 87, 4323 - 4341 HIKARI Ltd, www.m-hikari.com http://dx.doi.org/10.12988/ams.2014.45338
        Else
            Qrand = 1 - Qrand
            invphiQ = -10.0 / Math.Log10(41) * Math.Log10(1 - Math.Log10((-Math.Log10(Qrand) / Math.Log10(2))) / Math.Log10(22)) 'Applied Mathematical Sciences, Vol. 8, 2014, no. 87, 4323 - 4341 HIKARI Ltd, www.m-hikari.com http://dx.doi.org/10.12988/ams.2014.45338
        End If
        xrand = ((Math.Log(arand) - Math.Log(RAm) + RBu * invphiQ) / RBr) 'inside part of normal distribution of Fragility curve Equation 
        If (xrand > 0) Then
            phix = 1.0 / Math.Pow(2.0, Math.Pow(22.0, (1.0 - Math.Pow(41.0, xrand / 10.0)))) 'Applied Mathematical Sciences, Vol. 8, 2014, no. 87, 4323 - 4341 HIKARI Ltd, www.m-hikari.com http://dx.doi.org/10.12988/ams.2014.45338
        Else
            xrand = -xrand
            phix = 1.0 - 1.0 / Math.Pow(2.0, Math.Pow(22.0, (1.0 - Math.Pow(41.0, xrand / 10.0)))) 'Applied Mathematical Sciences, Vol. 8, 2014, no. 87, 4323 - 4341 HIKARI Ltd, www.m-hikari.com http://dx.doi.org/10.12988/ams.2014.45338
        End If
        FragilityEq = phix
        sx = arand.ToString & vbTab & Qrand.ToString & vbTab & invphiQ.ToString & vbTab & xrand.ToString & vbTab & phix.ToString
        Message.Add(sx)
    End Function
    ...
    Public Sub ReadTextDataFile(ByRef DataFileName As String)
        ...
        Dim aLine As String
        Dim sL() As String, Sep(2) As String
        Dim Haz() As String = {""}
        Dim Weight() As Single = {0.0}
        Dim Acc() As Single = {0.0}
        Dim Freq(,) As Single = {{0.0, 0.0}}
        Dim i, k, NumOfAcc As Integer
        Dim S As String        ' Hazard 내용을 기록할 String

        Message.Add("-- No Hazard Curves ---- ")

        Sep(0) = " " : Sep(1) = vbTab : Sep(2) = vbCrLf

        ' File 열어서 데이터 Line 수 count (using StreamReader)
        Dim CstrReader As System.IO.StreamReader = System.IO.File.OpenText(DataFileName)
        Do While (CstrReader.Peek() <> -1)   ' 첫줄까지 한줄씩 읽기
            aLine = CstrReader.ReadLine
            If (Not (aLine.Contains("!") Or (aLine = vbCrLf))) Then Exit Do
        Loop
        aLine = CstrReader.ReadLine   ' Hazard ID 한줄 읽기
        aLine = CstrReader.ReadLine   ' Hazard Weight 한줄 읽기
        k = 0
        Do While (CstrReader.Peek() <> -1)
            aLine = CstrReader.ReadLine   ' Acc와 Freq 한줄 읽기
            If (CstrReader.EndOfStream) Then Exit Do
            k += 1
        Loop
        CstrReader.Close()   ' File 닫기
        XHazard.NumOfAcc = k 'Start with k=0

        ' File 열기 (using StreamReader)
        Dim strReader As System.IO.StreamReader = System.IO.File.OpenText(DataFileName)

        Do While (strReader.Peek() <> -1)   ' 첫줄까지 한줄씩 읽기
        ...
        ' Data 읽기
        aLine = strReader.ReadLine   ' Hazard ID 한줄 읽기
        sL = aLine.Split(Sep, StringSplitOptions.RemoveEmptyEntries)     ' Word 단위로 분리하여 SL() 에 저장
        XHazard.NumOfHaz = UBound(sL) 'Start with 0
        ReDim Haz(XHazard.NumOfHaz)
        For i = 0 To XHazard.NumOfHaz
            Haz(i) = Trim(sL(i))
        Next

        ReDim Weight(XHazard.NumOfHaz)
        aLine = strReader.ReadLine   ' Hazard Weight 한줄 읽기
        sL = aLine.Split(Sep, StringSplitOptions.RemoveEmptyEntries)     ' Word 단위로 분리하여 SL() 에 저장
        For i = 0 To XHazard.NumOfHaz
            Weight(i) = Convert.ToSingle(sL(i))
        Next

        ReDim Acc(XHazard.NumOfAcc)
        ReDim Freq(XHazard.NumOfAcc, XHazard.NumOfHaz)

        k = 0
        Do While (strReader.Peek() <> -1)
            aLine = strReader.ReadLine   ' Acc와 Freq 한줄 읽기

            sL = aLine.Split(Sep, StringSplitOptions.RemoveEmptyEntries)     ' Word 단위로 분리하여 SL() 에 저장

            Acc(k) = Convert.ToSingle(sL(0))

            For i = 1 To UBound(sL)
                Freq(k, i - 1) = Convert.ToSingle(sL(i))
            Next
            k += 1
        Loop
        strReader.Close()   ' File 닫기

        ReDim XHazard.Haz(XHazard.NumOfHaz)
        ReDim XHazard.Weight(XHazard.NumOfHaz)
        ReDim XHazard.Acc(XHazard.NumOfAcc)
        ReDim XHazard.Freq(XHazard.NumOfAcc, XHazard.NumOfHaz)

        For i = 0 To XHazard.NumOfHaz
            XHazard.Haz(i) = Haz(i)
            XHazard.Weight(i) = Weight(i)
        Next

        For k = 0 To XHazard.NumOfAcc
            XHazard.Acc(k) = Acc(k)
            For i = 0 To XHazard.NumOfHaz
                XHazard.Freq(k, i) = Freq(k, i)

                S = XHazard.Acc(k).ToString & "  |  " & XHazard.Freq(k, i).ToString
                Message.Add(S)
            Next
        Next
        Message.Show()

...
Public Class ClassRawFile
    ...
    Public Sub ReadRawFile(ByRef RawFileName As String)
        ...
        Console.WriteLine(DateTime.Now.ToString)
        'For i = 1 To XData.NumOfEvent  ' Event 확인용 LHK
        '    Message.Add(i.ToString & "    " & XData.EventName(XData.index(i)))
        'Next
        'Message.Show()
        ...
        'If (XCutSet.NumOfCutSet < 10000) Then
        '    For i = 0 To nCutset - 1
        '        s = i.ToString & "   " & XCutSet.XaCutSet(XCutSet.index(i)).CutProba.ToString
        '        For j = 0 To XCutSet.XaCutSet(XCutSet.index(i)).NoElem - 1
        '            If (XCutSet.XaCutSet(XCutSet.index(i)).Elems(j) > 0) Then
        '                s = s & " = " & XData.EventName(XCutSet.XaCutSet(XCutSet.index(i)).Elems(j))
        '            Else
        '                s = s & " = " & "-" & XData.EventName(-XCutSet.XaCutSet(XCutSet.index(i)).Elems(j))
        '            End If
        '        Next
    ...
    Public Sub SortingEventNames(S1 As String, S2 As String)
        ...
        For isort = 0 To array_size
            a(isort) = XCutSet.XaCutSet(isort).CutProba
            'If a(i) = 0 Then
            '    Exit For
            'End If
        Next

        qsort(a, 0, array_size)
        ...
        Dim j, tempx As Integer
        ReDim XCutSet.index(XCutSet.NumOfCutSet) '기본사건 그룹별로 정리된 순서
        For i = 0 To XCutSet.NumOfCutSet - 1
            XCutSet.index(i) = i
        Next

        For i = 1 To XCutSet.NumOfCutSet - 1
            For j = i + 1 To 1 Step -1
                If XCutSet.XaCutSet(XCutSet.index(j)).CutProba > XCutSet.XaCutSet(XCutSet.index(j - 1)).CutProba Then
                    tempx = XCutSet.index(j)
                    XCutSet.index(j) = XCutSet.index(j - 1)
                    XCutSet.index(j - 1) = tempx
                End If
            Next j
        Next i
    End Sub
